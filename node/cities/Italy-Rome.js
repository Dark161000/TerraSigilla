...
async function getData(e) {
    ...
    let apiData = '';
    const soCoords = '41.76412969861398,12.336658174888774',
    neCoords = '42.05688764616363,12.738820902674647';
    ...
    if (!fs.existsSync(path.join(__dirname,'../../data/Italy-Rome.txt'))) {
        const header = '"id"::"dataPubblicazione"::"titolo"::"trattoEvento"::"evento"::"dettaglio"::"priorita"::"fonte"::"coordinateInizio"::"coordintateFine"\n';
        fs.writeFileSync(path.join(__dirname,'../../data/Italy-Rome.txt'), header, (err) => {if(err){console.error('Error writing to file: ', err)}});
        await e.send('preRenderTable', true, header);
    } else {
        const fileData = fs.readFileSync(path.join(__dirname,'../../data/Italy-Rome.txt'),'utf-8', (err) => {if(err){console.error('Error reading file: ', err)}});
