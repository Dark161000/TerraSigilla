...
async function getData(e) {
    ...
    let apiData = '';
    const soCoords = '43.437511524816244,10.717382696347396',
    neCoords = '44.24934314258925,11.80550694095917';
    ...
    if (!fs.existsSync(path.join(__dirname,'../../data/Italy-Florence.txt'))) {
        const header = '"id"::"dataPubblicazione"::"titolo"::"trattoEvento"::"evento"::"dettaglio"::"priorita"::"fonte"::"lineString"::"coordinate"\n';
        fs.writeFileSync(path.join(__dirname,'../../data/Italy-Florence.txt'), header, (err) => {if(err){console.error('Error writing to file: ', err)}});
        await e.send('preRenderTable', true, header);
    } else {
        const fileData = fs.readFileSync(path.join(__dirname,'../../data/Italy-Florence.txt'),'utf-8', (err) => {if(err){console.error('Error reading file: ', err)}});
    ...
    for (let i = 0; i < totalFeatures; i++) {
        ...
        if (!await nodeScripts.findDuplicates(e, path.join(__dirname,'../../data/Italy-Florence.txt'), info)) {
            fs.appendFileSync(path.join(__dirname,'../../data/Italy-Florence.txt'), info ,(err) => {if(err){console.error('Error writing to file: ', err)}});
