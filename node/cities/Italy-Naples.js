...
async function getData(e) {
    ...
    let apiData = '';
    const soCoords = '40.5099722682093,13.776393258043832',
    neCoords = '41.03912079884741,14.72608755577244';
    ...
    if (!fs.existsSync(path.join(__dirname,'../../data/Italy-Naples.txt'))) {
        const header = '"id"::"dataPubblicazione"::"titolo"::"trattoEvento"::"evento"::"dettaglio"::"priorita"::"fonte"::"lineString"::"coordinate"\n';
        fs.writeFileSync(path.join(__dirname,'../../data/Italy-Naples.txt'), header, (err) => {if(err){console.error('Error writing to file: ', err)}});
        await e.send('preRenderTable', true, header);
    } else {
        const fileData = fs.readFileSync(path.join(__dirname,'../../data/Italy-Naples.txt'),'utf-8', (err) => {if(err){console.error('Error reading file: ', err)}});
    ...
    for (let i = 0; i < totalFeatures; i++) {
        ...
        if (!await nodeScripts.findDuplicates(e, path.join(__dirname,'../../data/Italy-Naples.txt'), info)) {
            fs.appendFileSync(path.join(__dirname,'../../data/Italy-Naples.txt'), info ,(err) => {if(err){console.error('Error writing to file: ', err)}});
